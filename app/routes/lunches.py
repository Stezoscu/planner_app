...
lunches_bp = Blueprint("lunches", __name__)

SPREADSHEET_ID = os.getenv("SHEET_ID")
TEMPLATE_SHEET = "Standard Week"


# --- Utility reused from dinner.py ---
...
@lunches_bp.route("/", methods=["GET"])
@firebase_required
def get_lunches():
    """
    Returns the weekly lunch plan from the fixed range (I20:J27) in the Google Sheet.
    Query params:
    ...
    try:
        ...
        # === Fetch lunch range ===
        lunch_range = f"'{target_title}'!I20:J27"
        ...
        if not rows:
            return jsonify({"error": f"No data found in range {lunch_range}"}), 404
        ...
        for row in rows:
            ...
            meal = row[1].strip()
            if day and meal:
                lunches.append({"day": day, "meal": meal})

        print(f"✅ Lunch debug — target_title={target_title}, lunch_range={lunch_range}, rows={rows}")
        ...
        print(f"❌ Error in get_lunches: {e}")
